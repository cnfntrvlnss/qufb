...
listAllUser();//获取所有用户
//模块关闭通用方法
function openTabs(domId){
	document.getElementById(domId).classList.remove("collapsed-box");
	document.getElementById(domId).getElementsByTagName("i")[0].classList.add("fa-minus");
	document.getElementById(domId).getElementsByTagName("i")[0].classList.remove("fa-plus");
}

//模块打开通用方法
function colseTabs(domId){
	document.getElementById(domId).classList.add("collapsed-box");
	document.getElementById(domId).getElementsByTagName("i")[0].classList.remove("fa-minus");
	document.getElementById(domId).getElementsByTagName("i")[0].classList.add("fa-plus");
}

//模块关闭和打开
function tabSlide(slideButton){
	var boxInfoId = slideButton.attr("value");
	if(document.getElementById(boxInfoId).classList.contains('collapsed-box') == true){
		openTabs(boxInfoId)
	}else{
		colseTabs(boxInfoId);
	}
...
//返回问题列表 调用main.js中的方法
function backToList() {
	contentPageRender("/myQuestionSubmit")
}
...
function getQuestionInfo(){
     $.ajax({
           ...
           success: function(data){
                console.log("操作标志值是："+data.operateFlag);
                //判断问题状态
            ...
            error: function(data){
            	alertErrorInfo("提示","获取问题信息失败，请及时联系管理员解决问题");
                return;
            }
        });
}
    ...
    $.ajax({
      ...
      success: function(data){
        if(data =="success"){
            ...
            alert("保存失败，失败原因分析：数据格式异常，请联系管理员及时解决问题");
        }
       },
       error:function(){
            alert("保存失败，失败原因分析：数据格式异常，请联系管理员及时解决问题");
...
function btnGroup8(){
   ...
   $("#btnGroup7").addClass("btnGroup");//7组隐藏
    //模块关闭
   openTabs("submitQuestionDiv");
   openTabs("bugAuditSuggestDiv");
   openTabs("questionManagerDealDiv");
   openTabs("projectLiableSubmitDiv");
   openTabs("projectAuditDiv");
   openTabs("bugManageAuditDiv");
   openTabs("checkManageDiv");
}
...
function divGroup1(){
    ...
    $("#department1").css("display","none");//隐藏div
    $("#unit1").css("display","none");//隐藏div
    colseTabs("submitQuestionDiv");//关闭1 组
}
...
function divGroup2(){
    ...
    $("#department2").css("display","none");//隐藏div
    $("#unit2").css("display","none");//隐藏div
    colseTabs("bugAuditSuggestDiv");//关闭2组
}
...
function divGroup3(){
    ...
    $("#department3").css("display","none");//隐藏div
    $("#unit3").css("display","none");//隐藏div
    colseTabs("questionManagerDealDiv");//关闭3组
}
...
function divGroup4(){
    ...
    $("#department4").css("display","none");//隐藏div
    $("#unit4").css("display","none");//隐藏div
     colseTabs("projectLiableSubmitDiv");//关闭4组
}
...
function divGroup5(){
    ...
    $("#department5").css("display","none");//隐藏div
    $("#unit5").css("display","none");//隐藏div
     colseTabs("projectAuditDiv");//关闭5组
}
...
function divGroup6(){
    ...
    $("#department6").css("display","none");//隐藏div
    $("#unit6").css("display","none");//隐藏div
    colseTabs("bugManageAuditDiv");//关闭6组
}

//disable divGroup7
function divGroup7(){
    $("#verifySuggestion").attr("disabled","true");
     colseTabs("checkManageDiv");//关闭7组
}
//====================================获取下拉框的内容====================================================
